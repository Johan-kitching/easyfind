                ...
                @can('My Mechanic - Add')
                    ...
                    <div class="w-full text-center p-1">
                        <x-button outline primary class="dark:text-gray-400 hover:dark:text-white"
                                  onclick="Livewire.dispatch('openModal', {component: 'Mechanic.Create'})">
                    ...
                    @else
                        <div class="w-full text-center p-1">
                            <a href="{{route('payments')}}">Update your package to add more mechanics. <span class="text-blue-600 hover:text-blue-800"> Click Here.</span></a>
                        </div>
                    @endif
                        <div><span class="font-bold">Assets Remaining:</span> {{ Auth::user()->getAssetsRemainingAttribute() }} / {{Auth::user()->package->assets}}</div>
                @endcan
                <livewire:mechanic.mechanic-table :url="request()->url()"/>
            </div>
        </div>
    </div>
</x-app-layout>
