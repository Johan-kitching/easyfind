<div
    ...
    <div
        ...
        <div
            ...
            <x-modal-header>{{$machinery->id}} - {{$machinery->type->category}} - {{$machinery->type->type}} - {{$machinery->type->model}}</x-modal-header>
            <div class="relative flex-auto p-4" data-te-modal-body-ref x-data="{ submitButtonDisabled: 0}">
                <div class="flex flex-nowrap gap-4">
                    ...
                    <div class="my-2 w-fit text-center" x-transition>
                        <div class="w-fit">
                            <div x-data="{mainUrl:'{{$machinery->photos->first()->fullPath}}'}">
                    ...
                    <div class="text-left dark:text-gray-200 text-gray-800">
                        @if($machinery->user->companyName)
                            <div><span class="dark:text-gray-500 text-gray-400">Company:</span> <br>{{$machinery->user->companyName}}</div>
                        @endif
                        <div><span class="dark:text-gray-500 text-gray-400">Contact Name:</span> <br>{{$machinery->user->companyContact ?? $machinery->user->memberName}}</div>
                        <div><span class="dark:text-gray-500 text-gray-400">Contact Number:</span> <br>{{$machinery->user->companyNumber ?? $machinery->user->number}}</div>
                        @if($machinery->user->website)
                            <div><span class="dark:text-gray-500 text-gray-400">Website:</span> <br>{{$machinery->user->website}}</div>
                        @endif
                        @if($machinery->user->address)
