<div x-data="{
    ...
    resizeCanvas() {
        this.ratio = Math.max(window.devicePixelRatio || 2, 1);
...
}" @resize.window="resizeCanvas" class="p-5"  {{ $attributes }} wire:ignore >


    <canvas x-ref="canvas" class="w-full h-[300px] border-2 border-gray-300 border-dashed rounded-md "></canvas>

    <div class="flex mt-2 space-x-2">
        <x-button rounded negative label="Clear" x-on:click.prevent="clear()" />
        <x-button rounded positive label="Save" x-on:click.prevent="save()" />
        <span x-data="{
            open: false,
            saved(e) {
