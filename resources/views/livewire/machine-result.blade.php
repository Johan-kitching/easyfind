<x-card title="{{$machinery->category}} - {{$machinery->type}}" rounded="3xl" shadow="2xl" class="hover:shadow-none max-w-[288px]" x-data="{mainUrl:'{{$machinery->photos?->first()?->fullPath}}'}">
    <div>
        <div>
    ...
    <x-slot name="footer" class="flex gap-4 items-start justify-end text-blue-500 place-content-between place-items-center" style="place-content: space-between; place-items: center;">
        <div>{{round($machinery->distance,2)}}Km away</div>
        <x-button wire:click="dispatch('openModal', {component: 'Machinery.view', arguments: {{ json_encode([$machinery->id]) }} })">
            {{ __('View') }}
        </x-button>
    </x-slot>
</x-card> 
