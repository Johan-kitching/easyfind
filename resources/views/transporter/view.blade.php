<div
    ...
    <div
        ...
        <div
            ...
            <x-modal-header>{{$transporter->id}} - {{$transporter->type->name}}</x-modal-header>
            <div class="relative flex-auto p-4" data-te-modal-body-ref x-data="{ submitButtonDisabled: 0}">
                <div class="flex flex-nowrap gap-4">
                    ...
                    <div class="my-2 w-fit text-center" x-transition>
                        <div class="w-fit">
                            <div x-data="{mainUrl:'{{$transporter->photos->first()->fullPath}}'}">
                                ...
                                <div id="SmallImage" class="justify-end">
                    ...
                    <div class="text-left dark:text-gray-200 text-gray-800">
                        @if($transporter->user->companyName)
                            <div><span class="dark:text-gray-500 text-gray-400">Company:</span> <br>{{$transporter->user->companyName}}</div>
                        @endif
                        <div><span class="dark:text-gray-500 text-gray-400">Contact Name:</span> <br>{{$transporter->user->companyContact ?? $transporter->user->memberName}}</div>
                        <div><span class="dark:text-gray-500 text-gray-400">Contact Number:</span> <br>{{$transporter->user->companyNumber ?? $transporter->user->number}}</div>
                        @if($transporter->user->website)
                            <div><span class="dark:text-gray-500 text-gray-400">Website:</span> <br>{{$transporter->user->website}}</div>
                        @endif
                        @if($transporter->user->address)
                            <div><span class="dark:text-gray-500 text-gray-400">Address:</span> <br>{{$transporter->user->address}}</div>
                        @endif
                        @if($transporter->address)
                            <div><span class="dark:text-gray-500 text-gray-400">Current Address:</span> <br>{{$transporter->user->address}}</div>
                        @endif
                        @if($transporter->description)
                            <div><span class="dark:text-gray-500 text-gray-400">Description:</span> <br>{{$transporter->description}}</div>
                        @endif
                    </div>
                </div>
                <div
